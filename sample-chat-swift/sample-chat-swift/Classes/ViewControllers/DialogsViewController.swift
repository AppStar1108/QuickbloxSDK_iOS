...
class DialogsViewController: UITableViewController, QMChatServiceDelegate, QMChatConnectionDelegate, QMAuthServiceDelegate {
    ...
    @IBAction func logoutAction() {
        ...
        SVProgressHUD.showWithStatus("SA_STR_LOGOUTING".localized, maskType: SVProgressHUDMaskType.None)
        
        ServicesManager.instance().logOutUser().continueWithBlock {[weak self] (task:BFTask!) -> AnyObject? in
            ...
            if (task.result == nil) {
                ...
                ServicesManager.instance().authService.removeDelegate(strongSelf)
